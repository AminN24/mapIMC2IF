...
def visualize(iteration, error, X, Y, ax):
    plt.cla()
    ax.scatter(X[:, 0],  X[:, 1], color='red', label='IMC')
    ax.scatter(Y[:, 0],  Y[:, 1], color='blue', label='IF')
...
def mapping(X, Y, max_iterations=700, tolerance=1e-4, w=1e-3):
    '''
    Mapping IMC cell coordinates to IF.
    Attributes:
    ----------
        X: numpy array
            NxD array of target points.
...
def main():
    ...
    imc_dir = sys.argv[2]
    id = int(sys.argv[3]) 
    names = os.listdir(if_path)
    print("Sample name: ", names[id])
    
    # X is red
    # Y is blue
    X = np.loadtxt(f"{imc_path}/{names[id]}") 
    Y = np.loadtxt(f"{if_path}/{names[id]}")
    ...
    reg = RigidRegistration(**{'X': X, 'Y': Y, 'max_iterations': 700,
                               'tolerance': 1e-4, 'w':2e-2})
